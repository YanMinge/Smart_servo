...
typedef struct
{
     ...
     * |        |          |trigger, the trigger source is control de-bounce. If the interrupt is by level trigger, the input source is
...
typedef struct
{
     ...
     * |        |          |This bit is set to logic 1 while RS485_ADD_EN (UA_ALT_CSR[15]) is set to 1 to enable
     * |        |          |Address detection mode and receive detect a data with an address bit (bit 9 = 1).
