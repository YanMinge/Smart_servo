...
volatile float g_pre_temperature;
...
int main(void)
{
    ...
    while(1)
    {
		...
		if(g_block_no != 0) 
		{
			...
			{
				if(g_temperature != g_pre_temperature)
				{
...
void send_sensor_report_offline(void)
{
	...
	int32_t temp_value = (int32_t)g_temperature;
	int16_t uniform_value =  line_map(temp_value, UNIFORM_MIN, UNIFORM_MAX, TEMPERATURE_MIN, TEMPERATURE_MAX);
