...
angle_offset_data_struct_type angle_offset_data_struct = {0};
...
SpiFlashOpResult write_cmd_mode_to_flash(uint32_t cmd_mode)
{
  ...
  result = spi_flash_write(USER_DATA_CMD_MODE_START_ADDR,(uint32_t *)&cmd_mode_data_struct.cmd_mode_start,sizeof(cmd_mode_data_struct_type));
  ...
  result = spi_flash_write(USER_DATA_CMD_MODE_START_ADDR + DATA_FLASH_PAGE_1_OFFSET,(uint32_t *)&cmd_mode_data_struct.cmd_mode_start,sizeof(cmd_mode_data_struct_type));
  if(result == SPI_FLASH_RESULT_ERR)
  {
...
uint32_t flash_read_data_mode(void)
{
     ...
     (spi_flash_read(USER_DATA_CMD_MODE_END_ADDR,&cmd_mode_end,4) == SPI_FLASH_RESULT_OK))
  {
            ...
            (spi_flash_read(USER_DATA_CMD_MODE_END_ADDR + DATA_FLASH_PAGE_1_OFFSET,&cmd_mode_end,4) == SPI_FLASH_RESULT_OK))
    {
      ...
      {
#ifdef DATA_FLASH_DEBUG
        uart_printf(UART0,"write_cmd_mode_to_flash 1\r\n");
#endif /* DATA_FLASH_DEBUG */
  ...
  {
#ifdef DATA_FLASH_DEBUG
    uart_printf(UART0,"write_cmd_mode_to_flash 2\r\n");
#endif /* DATA_FLASH_DEBUG */
    write_cmd_mode_to_flash(FIRMATA_DATA_MODE);
  }
  return cmd_mode;
...
SpiFlashOpResult write_angle_offset_to_flash(uint32_t angle_offset)
{
  SpiFlashOpResult result = SPI_FLASH_RESULT_OK;

  angle_offset_data_struct.angle_offset_start = USER_DATA_ANGLE_OFFSET_CHECK_START;
  angle_offset_data_struct.angle_offset_end = USER_DATA_ANGLE_OFFSET_CHECK_END;
  angle_offset_data_struct.angle_offset = angle_offset;
  
#ifdef DATA_FLASH_DEBUG
  uart_printf(UART0,"angle_offset Write primary partition!\r\n");
#endif /* DATA_FLASH_DEBUG */

  result = spi_flash_write(USER_DATA_ANGLE_OFFSET_START_ADDR,(uint32_t *)&angle_offset_data_struct.angle_offset_start,sizeof(angle_offset_data_struct_type));
  if(result == SPI_FLASH_RESULT_ERR)
  {
    uart_printf(UART0,"angle_offset Write primary partition failed!\r\n");
  }

#ifdef DATA_FLASH_DEBUG
  uart_printf(UART0,"angle_offset Write backup partition!\r\n");
#endif /* DATA_FLASH_DEBUG */

  result = spi_flash_write(USER_DATA_ANGLE_OFFSET_START_ADDR + DATA_FLASH_PAGE_1_OFFSET,(uint32_t *)&angle_offset_data_struct.angle_offset_start,sizeof(angle_offset_data_struct_type));
  if(result == SPI_FLASH_RESULT_ERR)
  {
    uart_printf(UART0,"angle_offset Write backup partition failed!\r\n");
  }
  return result;
}

uint32_t flash_read_angle_offset(void)
{
  uint32_t angle_offset_start;
  uint32_t angle_offset_end;
  uint32_t angle_offset = 0;
  if((spi_flash_read(USER_DATA_ANGLE_OFFSET_START_ADDR,&angle_offset_start,4) == SPI_FLASH_RESULT_OK) &&
     (spi_flash_read(USER_DATA_ANGLE_OFFSET_END_ADDR,&angle_offset_end,4) == SPI_FLASH_RESULT_OK))
  {
    if((angle_offset_start + angle_offset_end == DATA_FULL_RANK) &&
       (spi_flash_read(USER_DATA_ANGLE_OFFSET,&angle_offset,4) == SPI_FLASH_RESULT_OK))
    {
#ifdef DATA_FLASH_DEBUG
      uart_printf(UART0,"read data mode primary partition,angle_offset(%d)\r\n",angle_offset);
#endif /* DATA_FLASH_DEBUG */
    }
    else if((spi_flash_read(USER_DATA_ANGLE_OFFSET_START_ADDR + DATA_FLASH_PAGE_1_OFFSET,&angle_offset_start,4) == SPI_FLASH_RESULT_OK) &&
            (spi_flash_read(USER_DATA_ANGLE_OFFSET_END_ADDR + DATA_FLASH_PAGE_1_OFFSET,&angle_offset_end,4) == SPI_FLASH_RESULT_OK))
    {
      if((angle_offset_start + angle_offset_end == DATA_FULL_RANK) &&
         (spi_flash_read(USER_DATA_ANGLE_OFFSET + DATA_FLASH_PAGE_1_OFFSET,&angle_offset,4) == SPI_FLASH_RESULT_OK))
      {
#ifdef DATA_FLASH_DEBUG
        uart_printf(UART0,"read data backup partition 1,angle_offset(%d)\r\n",angle_offset);
#endif /* DATA_FLASH_DEBUG */
      }
      else
      {
#ifdef DATA_FLASH_DEBUG
        uart_printf(UART0,"write_angle_offset_to_flash 1\r\n");
#endif /* DATA_FLASH_DEBUG */
        write_angle_offset_to_flash(0);
      }
    }
  }
  else if((spi_flash_read(USER_DATA_ANGLE_OFFSET_START_ADDR  + DATA_FLASH_PAGE_1_OFFSET,&angle_offset_start,4) == SPI_FLASH_RESULT_OK) &&
          (spi_flash_read(USER_DATA_ANGLE_OFFSET_END_ADDR + DATA_FLASH_PAGE_1_OFFSET,&angle_offset_end,4) == SPI_FLASH_RESULT_OK))
  {
    ...
    {
#ifdef DATA_FLASH_DEBUG
      uart_printf(UART0,"read data backup partition 2,angle_offset(%d)\r\n",angle_offset);
#endif /* DATA_FLASH_DEBUG */
    }
  }
  else
  {
#ifdef DATA_FLASH_DEBUG
    uart_printf(UART0,"write_angle_offset_to_flash 2\r\n");
#endif /* DATA_FLASH_DEBUG */
    write_angle_offset_to_flash(0);
  }
  return angle_offset;
...
SpiFlashOpResult flash_write_angle_offset(uint32_t angle_offset)
{
  SpiFlashOpResult result = SPI_FLASH_RESULT_OK;

  uint32_t angle_offset_start;
  uint32_t angle_offset_end;
  uint32_t angle_offset_temp = 0;

  if((spi_flash_read(USER_DATA_ANGLE_OFFSET_START_ADDR,&angle_offset_start,4) == SPI_FLASH_RESULT_OK) &&
     (spi_flash_read(USER_DATA_ANGLE_OFFSET_END_ADDR,&angle_offset_end,4) == SPI_FLASH_RESULT_OK) &&
     (spi_flash_read(USER_DATA_ANGLE_OFFSET,&angle_offset_temp,4) == SPI_FLASH_RESULT_OK) &&
     (angle_offset_start + angle_offset_end == DATA_FULL_RANK) &&
     (angle_offset_temp == angle_offset))
  {
#ifdef DATA_FLASH_DEBUG
    uart_printf(UART0,"angle_offset No need change 1\r\n");
#endif /* DATA_FLASH_DEBUG */
    return result;
  }

  if((spi_flash_read(USER_DATA_ANGLE_OFFSET_START_ADDR + DATA_FLASH_PAGE_1_OFFSET,&angle_offset_start,4) == SPI_FLASH_RESULT_OK) &&
     (spi_flash_read(USER_DATA_ANGLE_OFFSET_END_ADDR + DATA_FLASH_PAGE_1_OFFSET,&angle_offset_end,4) == SPI_FLASH_RESULT_OK) &&
     (spi_flash_read(USER_DATA_ANGLE_OFFSET + DATA_FLASH_PAGE_1_OFFSET,&angle_offset_temp,4) == SPI_FLASH_RESULT_OK) &&
     (angle_offset_start + angle_offset_end == DATA_FULL_RANK) &&
     (angle_offset_temp == angle_offset))
  {
#ifdef DATA_FLASH_DEBUG
    uart_printf(UART0,"angle_offset No need change 2\r\n");
#endif /* DATA_FLASH_DEBUG */
    return result;
  }

  result = write_angle_offset_to_flash(angle_offset);
